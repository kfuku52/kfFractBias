...
from kffractbias.selfscan import quota_constraints, read_self_hits, scan_self, select_quota
...
def test_self_quota_combines_both_axes():
    ...
    assert quota_constraints(blocks, genes, 1) == [(0, 1), (0, 2), (1, 2)]
    assert quota_constraints(blocks, genes, 2) == []
...
@pytest.mark.parametrize("distance,expected", [(299, 0), (300, 4), (301, 4)])
def test_self_diagonal_boundary_and_liftover(tmp_path, distance, expected):
    ...
    hits = read_self_hits(blast, genes, 300)
    assert sum(map(len, hits.values())) == expected
...
@pytest.mark.parametrize("padding_chrom", [None, "chr0", "chrC", "chrZ"])
